...
(function () {
  ...
  const musicSource = document.getElementById("audio-source");
  const musicControl = document.getElementById("music-control");
  const iconPlay = document.getElementById("music-icon-play");
  const iconPause = document.getElementById("music-icon-pause");
  let isPlaying = false;

  // Masukkan URL musik dari CONFIG
  if (bgMusic && musicSource && CONFIG.musicUrl) {
  ...
  // Pasang event listener pada tombol musik melayang
  if (musicControl) {
  ...
  if (btnOpen && gate && main) {
    btnOpen.addEventListener("click", function () {
      // Sembunyikan gerbang dan tampilkan isi undangan
      ...
      // Mainkan musik secara otomatis setelah tombol diklik
      if (bgMusic && CONFIG.musicUrl) {
        bgMusic.play().then(() => {
          ...
          console.warn("Autoplay ditolak atau error:", err);
        });
        
        // Tampilkan tombol kontrol musik di pojok kanan bawah
        if (musicControl) {
  ...
  if (footerLogo) {
    if (CONFIG.logoUrl && CONFIG.logoUrl.trim() !== "") {
      ...
      footerLogo.style.display = "block"; // Pastikan logo muncul jika ada URL-nya
    } else {
      footerLogo.style.display = "none"; // Sembunyikan elemen jika URL logo kosong
    }
  }

  // ---- Memuat Link Sosial Media ----
  ...
  // Atur link WA
  if (linkWa && CONFIG.socialMedia && CONFIG.socialMedia.whatsapp) {
    linkWa.href = CONFIG.socialMedia.whatsapp;
  } else if (linkWa) {
    linkWa.style.display = "none"; // Sembunyikan kalau link di config kosong
  }

  // Atur link Instagram
  if (linkIg && CONFIG.socialMedia && CONFIG.socialMedia.instagram) {
    linkIg.href = CONFIG.socialMedia.instagram;
  } else if (linkIg) {
    linkIg.style.display = "none";
  }

  // Atur link TikTok
  if (linkTiktok && CONFIG.socialMedia && CONFIG.socialMedia.tiktok) {
    linkTiktok.href = CONFIG.socialMedia.tiktok;
  } else if (linkTiktok) {
